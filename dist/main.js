  ...
  "use strict";
    ...
    // Firebse
        ...
        const userData = querySnapshot.forEach((doc)=>{
            if(doc.id === window.localStorage.getItem('userId')){
        ...
        if(window.userData){
          window.localStorage.setItem('defaultProjects',window.userData.defaultProjects)
          window.localStorage.setItem('projects',window.userData.projects)
        }


        const updateFirebase = async(pr,dpr)=>{
          try {
            const docRef = await setDoc(doc(db, "todos",localStorage.getItem('userId')), {
              projects:pr,
              defaultProjects:dpr
            });
          } catch (e) {
        ...
        setInterval(()=>{
          let pr = localStorage.getItem('projects')
          let dpr = localStorage.getItem('defaultProjects')

          if(!window.userData){
            updateFirebase(pr,dpr)
          }

          else if(window.userData.projects != pr || window.userData.defaultProjects != dpr){
            updateFirebase(pr,dpr)
          }
        },1000)
